...
class ConfigRepository(private val context: Context) {
    ...
    val configFlow: Flow<ApiConfig> = context.dataStore.data.map { prefs ->
        ApiConfig(
            llmBaseUrl = prefs[LLM_BASE_URL]?.ifEmpty { null } ?: "https://api.xiaomimimo.com",
            llmApiKey = prefs[LLM_API_KEY]?.ifEmpty { null } ?: "sk-cf2i5hjyod6wkhy6au3ul7whpxrhahoiou1929hlyny00wdf",
            llmModel = prefs[LLM_MODEL]?.ifEmpty { null } ?: "mimo-v2.5",
            maxContextTokens = prefs[MAX_CONTEXT_TOKENS]?.toIntOrNull() ?: 200000,
            asrProvider = prefs[ASR_PROVIDER]?.ifEmpty { null } ?: "vosk",
            asrBaseUrl = prefs[ASR_BASE_URL] ?: "",
            asrApiKey = prefs[ASR_API_KEY] ?: "",
            asrLanguage = prefs[ASR_LANGUAGE]?.ifEmpty { null } ?: "zh-CN",
            asrVoskModelId = prefs[ASR_VOSK_MODEL_ID] ?: "",
            ttsProvider = prefs[TTS_PROVIDER]?.ifEmpty { null } ?: "mimo",
            ttsBaseUrl = prefs[TTS_BASE_URL]?.ifEmpty { null } ?: "https://api.xiaomimimo.com",
            ttsApiKey = prefs[TTS_API_KEY]?.ifEmpty { null } ?: "sk-cf2i5hjyod6wkhy6au3ul7whpxrhahoiou1929hlyny00wdf",
            ttsModel = prefs[TTS_MODEL]?.ifEmpty { null } ?: "mimo-v2.5-tts-voiceclone",
